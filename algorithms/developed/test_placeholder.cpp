...
int test_algo(unsigned char *P, int m, unsigned char *T, int n, int k, float *time)
{
    ...
    // this array size must be >= 1 + MAX_PAT_LEN // 16
    __m128i packed_pattern[1 + MAX_PAT_LEN / 16];
    __m128i packed_text;
    bool eq;
    ...
    {
        while (z[word(get0_i128(packed_positions)) & mask] != 0 && z[word(get1_i128(packed_positions)) & mask] != 0 && z[word(get2_i128(packed_positions)) & mask] != 0)
        {
            packed_positions = _mm_sub_epi32(packed_positions, fill_m);
        }
