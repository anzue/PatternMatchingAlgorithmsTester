...
int search_large_aoso4_aoso4(unsigned char *x, int m, unsigned char *y, int n, int q);

int search_aoso4(unsigned char *x, int m, unsigned char *y, int n) {
    unsigned int B[MAX_SIGMA], D, h, mm, tmp, not_mm;
    int i, j, count;
    int q=4;

    if(m<=q) return -1;   
    if(m>32) return search_large_aoso4_aoso4(x,m,y,n,q);
    ...
    while (j < n) {
        D = ((D & ~mm)<<1)|B[y[j]];
        if ((D & mm) != mm)
            verify_aoso4_aoso4(y, j, n, x, m, q, D, mm, &count);
        j += q;
    }
    return count;
}
...
void verify_aoso4_large_aoso4(unsigned char *y, int j, int n, unsigned char *x, int m, int q, unsigned int D, unsigned int mm, int *count, int p_len) {
    unsigned int s, tmp;
    int c, k, i;
    D = (D & mm)^mm;
    while (D != 0) {
...
int search_large_aoso4_aoso4(unsigned char *x, int m, unsigned char *y, int n, int q) {
    ...
    while (j < n) {
        D = ((D & ~mm)<<1)|B[y[j]];
        if ((D & mm) != mm)
            verify_aoso4_large_aoso4(y, j, n, x, m, q, D, mm, &count, p_len);
        j += q;
    }
    return count;
}
