...
void suffixes_ag(unsigned char *x, int m, int *suff) {
    int f, g, i;
    suff[m - 1] = m;
    g = m - 1;
    for (i = m - 2; i >= 0; --i) {
...
void preBmGs_agAG(unsigned char *x, int m, int bmGs[], int suff[]) {
    int i, j;
    suffixes_ag(x, m, suff);
