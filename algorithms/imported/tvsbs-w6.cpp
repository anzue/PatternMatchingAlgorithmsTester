...
void TVSBSpreBrBc_tvsbs_w6_tvsbs_w6(unsigned char *x, int m, int brBc[MAX_SIGMA][MAX_SIGMA]) {
   int a, b, i;
   for (a = 0; a < SIGMA; ++a)
      for (b = 0; b < SIGMA; ++b)
         brBc[a][b] = m + 2;
 ...
 int tvsbs_w6(unsigned char *x, int m, unsigned char *y, int n){
   ...
   count = 0;
   int mp1 = m+1, mm1=m-1;
   TVSBSpreBrBc_tvsbs_w6_tvsbs_w6(x, m,  BrBcR);
   TVSBSpreBrBc_tvsbs_w6_tvsbs_w6(xr, m, BrBcL);
