...
int faoso4(unsigned char *x, int m, unsigned char *y, int n) {
   ...
   while (j < n) {
      ...
      D = (D<<1)|(B[y[j+q]]&~masq);
      if ((D & mm) != mm)
         verify_faoso4(y, j+uq1, n, x, m, q, u, D, mm, &count);
      D &= ~mm;
      j += uq;
   }
   return count;
...
void verify_faoso4_large(unsigned char *y, int j, int n, unsigned char *x, int m, int q, int u, unsigned int D, unsigned int mm, int *count, int p_len) {
   int s, c, mq, v, z, i, k;
...
int large_faoso4(unsigned char *x, int m, unsigned char *y, int n, int q) {
   ...
   while (j < n) {
      ...
      D = (D<<1)|(B[y[j+q]]&~masq);
      if ((D & mm) != mm)
         verify_faoso4_large(y, j+uq1, n, x, m, q, u, D, mm, &count, p_len);
      D &= ~mm;
      j += uq;
   }
   return count;
