...
int search_aoso6(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    while (j < n) {
        D = ((D & ~mm)<<1)|B[y[j]];
        if ((D & mm) != mm)
            verify_aoso6(y, j, n, x, m, q, D, mm, &count);
        j += q;
    }
    return count;
}
...
void verify_aoso6_large_aoso6(unsigned char *y, int j, int n, unsigned char *x, int m, int q, unsigned int D, unsigned int mm, int *count, int p_len) {
    unsigned int s, tmp;
    int c, k, i;
    D = (D & mm)^mm;
    while (D != 0) {
...
int search_large_aoso6(unsigned char *x, int m, unsigned char *y, int n, int q) {
    ...
    while (j < n) {
        D = ((D & ~mm)<<1)|B[y[j]];
        if ((D & mm) != mm)
            verify_aoso6_large_aoso6(y, j, n, x, m, q, D, mm, &count, p_len);
        j += q;
    }
    return count;
}
