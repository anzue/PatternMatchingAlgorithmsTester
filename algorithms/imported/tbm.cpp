...
void preBmBc_tbm(unsigned char *x, int m, int bmBc[]) {
   int i;
   for (i = 0; i < SIGMA; ++i)
      bmBc[i] = m;
   for (i = 0; i < m - 1; ++i)
...
void suffixes_tbm(unsigned char *x, int m, int *suff) {
   int f, g, i;
   suff[m - 1] = m;
   g = m - 1;
   for (i = m - 2; i >= 0; --i) {
...
void preBmGs_tbm(unsigned char *x, int m, int bmGs[]) {
   int i, j, suff[MAX_PAT_LEN];
   suffixes_tbm(x, m, suff);
   for (i = 0; i < m; ++i)
      bmGs[i] = m;
   j = 0;
   for (i = m - 1; i >= 0; --i)
...
int search_tbm(unsigned char *x, int m, unsigned char *y, int n) {
   ...
   /* Preprocessing */
   preBmGs_tbm(x, m, bmGs);
   preBmBc_tbm(x, m, bmBc);
