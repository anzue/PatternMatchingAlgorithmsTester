...
void preBmBc_akc(unsigned char *x, int m, int *bmBc) {
    int i;
...
void suffixes_akc_(unsigned char *x, int m, int *suff) {
    int f, g, i;
    suff[m - 1] = m;
    g = m - 1;
    for (i = m - 2; i >= 0; --i) {
...
void preSLink(unsigned char *x, int m, int *suff, int *pref, int *slink) {
    int i, f, k, ell, cptr[MAX_SIGMA], *len, *link, *loc, *next, *owner, *preLoc;
    int a;
    unsigned char *c;
...
int akc(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    preBmBc_akc(x, m, bmBc);
    suffixes_akc_(x, m, suff);
    prePref(m, suff, pref);
    lsp = preLsp(m, suff);
